...
- 앱은 repo 안의 gate contract를 바꾸지 않지만, watcher를 통해 approval state 파일을 읽고 `local_wait`, `pending`, `resolved`, `timeout` 같은 상태를 실제로 갱신합니다.
...
- 역할:
  - Telegram / ntfy 설정
  - Windows 사용자 환경 변수 저장
  - monitored repo 등록
  - away mode 전환
  - watcher 설치 / 제거 / 즉시 실행
  - runtime home의 `presence.json`, `repo_registry.json`, `watcher_last_summary.json`, `harness_admin.log` 관리
...
이 값들은 보통 앱의 `Settings` 탭에서 저장하며, 저장 시 Windows 사용자 환경 변수로 기록됩니다.
...
- 필요하면 Telegram 원문 메시지에 recorded confirmation reply도 다시 보냅니다.
- 최근 watcher 실행 요약은 `watcher_last_summary.json`, 로그는 `harness_admin.log`에서 확인합니다.
