...
- starter template: 새 프로젝트에 복사해 시작할 full starter source인 [`templates/project/`](templates/project)
- version closeout reset source: reset 대상 7개 artifact의 canonical source인 [`templates/version_reset/artifacts/`](templates/version_reset/artifacts)

즉, root의 `.agents/*`는 이 저장소 운영용이고, downstream 프로젝트에 배포할 표준 source는 `templates/project/*`와 `templates/version_reset/artifacts/*`에서 관리합니다.
...
- `templates/project/PROJECT_WORKFLOW_MANUAL.md`: starter template 사용자를 위한 인간용 설명서

위 두 문서는 운영 프로세스 입력 문서가 아니며, live state나 gate 판단 근거로 사용하지 않습니다.

## 저장소 구성 빠른 지도
- [`.agents/`](.agents): 이 저장소 자체를 운영하는 live 문서, skill, script
- [`templates/project/`](templates/project): downstream 프로젝트에 배포할 full starter root
...
- [`templates/project/PROJECT_WORKFLOW_MANUAL.md`](templates/project/PROJECT_WORKFLOW_MANUAL.md): 실제 starter template용 사용자 설명서
...
4. downstream 공통 동작이면 `templates/project/*`, 필요 시 `templates/version_reset/artifacts/*`를 canonical source로 갱신합니다.
