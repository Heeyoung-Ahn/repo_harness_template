...
description: 코드, UI, 테스트, 리뷰, 문서 변경 뒤 이 워크스페이스의 기능 관련 아티팩트를 동기화하는 스킬. 기능 구현이나 수정으로 제품 동작, 아키텍처, UI 흐름, QA 상태, review 범위, dependency audit 결과, handoff 상태가 바뀌어 `.agents/artifacts/*.md`와 관련 운영 문서를 함께 최신화해야 할 때 사용한다. `README.md`와 `PROJECT_WORKFLOW_MANUAL.md`는 비운영 문서이므로 기본 sync 대상에서 제외하고, 사용자가 명시적으로 요청한 경우에만 별도로 다룬다. 특히 대형 PR, dirty worktree, active lock, 부분 승인 상태에서 문서를 과장 없이 맞춰야 할 때 반드시 사용한다.
---
...
- 운영 아티팩트와 함께 외부 노출 문서까지 정리해야 하는지 명시적으로 판단해야 한다.
...
- 사용자 안내 변경: `docs/*` 또는 명시적으로 요청된 외부 문서
...
5. 지원 문서: `docs/*`, `ai_features_summary.md`, 명시적으로 요청된 외부 문서
...
- `docs/*`, `ai_features_summary.md`, 명시적으로 요청된 외부 문서가 사용자에게 오해를 주지 않는지
