...
## Quick Read
- 이 starter baseline의 목표: profile/runtime/pack truth를 먼저 고정하고, 필요 시 optional visibility와 rollout-ready dry-run/reporting까지 단계적으로 닫을 수 있게 한다
- 현재 stage: Planning and Architecture
- 현재 iteration: Kickoff - Contract Freeze
...
- 지금 바로 필요한 검증: requirement / architecture / plan sync, profile/pack/runtime baseline 확정
- 남아 있는 release gate (manual / dependency / compliance): planning iteration에서는 none
- optional self-hosting tool boundary check: `.omx/*`나 visibility tool은 starter 기본 동작을 바꾸지 않아야 한다
- enterprise-governed pack / governance control check: pack activation prerequisite와 dry-run/reporting defer policy를 요구사항에 명시하고, optional guardrail field는 core path를 무겁게 만들지 않게 둔다
- 현재 green level target: `Targeted`
- branch freshness precheck: starter baseline을 프로젝트 요구사항에 맞게 먼저 정리한다
...
## Current Iteration
- Iteration name: Kickoff - Contract Freeze
- Scope: 운영 프로필, runtime contract, optional pack, rollout defer policy를 먼저 고정한다
...
## Changelog
- [2026-04-07] Template Maintainer: starter implementation plan baseline을 hybrid runtime / governed pack / rollout defer 기준으로 강화했다.
- [2026-04-08] Template Maintainer: starter implementation plan에 task packet context contract, recurrence review, AI-specific review checklist, optional guardrail regression baseline을 추가했다.

## Objective
- 이번 버전의 목표: 프로젝트별 요구사항을 template baseline 위에 안전하게 올리고, optional pack과 rollout timing을 초반에 고정한다
...
|---|---|---|---|---|
| TEMPLATE-02 | `PLN-01`, `PLN-02`, `PLN-03` | runtime boundary, governed pack, rollout defer 기본 검증 추가 | Synced | starter baseline refresh |
| TEMPLATE-03 | `PLN-01`, `PLN-02`, `PLN-03`, `DEV-05`, `TST-03` | task packet contract, recurrence review, shared checklist, optional guardrail regression 추가 | Synced | starter reinforcement baseline refresh |
| TEMPLATE-04 | `PLN-03`, `TST-02`, `REL-02` | browser-based web test gate와 deploy preflight evidence 추가 | Synced | starter browser-validation baseline refresh |
...
| DEV-05 | reinforcement contract를 starter baseline에 반영한다 | In: task packet context contract, recurrence review, shared checklist baseline, optional guardrail field. Out: new control plane or mandatory enterprise burden | root/starter baseline sync pass, recurrence review는 existing artifact에만 기록되고 optional guardrail field는 dormant placeholder로 유지 | starter artifacts, starter skills, runtime contracts, review/deploy artifacts | starter default가 무거워지거나 새 top-level truth가 생기면 Planner/User로 올린다 |
| TST-03 | starter reinforcement regression을 검증한다 | In: validator, mirror sync, optional visibility risk signal regression. Out: actual rollout | validator pass, root/starter mirror sync, optional signal remains read-only | validator/test artifacts, current-state/task artifacts | contract mismatch나 parser break가 나면 Planner/Developer로 올린다 |
