...
- `IMPLEMENTATION_PLAN.md > Task Packet Ledger`
...
- [ ] non-trivial change의 `Primary Change Type`, `Impact Tier`, `Decision Log Entry` trigger를 먼저 확인했는가?
- [ ] `Self-Review Summary`가 남아 있고, 없다면 trivial exemption reason이 타당한가?
...
- [ ] 이번 변경이 전역 구조 일관성이나 기존 architecture invariant를 깨지 않는가?
- [ ] 국소 수정처럼 보이지만 실제로는 다른 모듈의 contract를 암묵적으로 바꾸고 있지 않은가?
- [ ] `architecture-change` 또는 qualifying `refactor`인데 `DECISION_LOG.md`와 full impact contract가 빠지지 않았는가?
- [ ] boundary / shared contract / hotspot이 바뀌었는데 `SYSTEM_CONTEXT.md`가 stale 상태로 남아 있지 않은가?
...
- [ ] domain term / lifecycle / invariant / exception rule이 바뀌었는데 `DOMAIN_CONTEXT.md`와 impact check가 함께 갱신됐는가?
...
- [ ] 같은 문제를 또 다른 파일/레이어에서 무통제 재생성한 흔적이 없는가?
- [ ] 불필요한 추상화, premature genericization, 이름만 그럴듯한 wrapper가 늘어나지 않았는가?
...
### 8. AI-generated Delta 전용 점검
- [ ] AI가 전체 시스템 맥락을 놓친 채 로컬한 결정만 내린 흔적이 없는가?
- [ ] 중복 코드와 불필요한 추상화가 빠르게 누적되는 패턴이 없는가?
- [ ] 이번 변경이 비관련 회귀를 유발할 수 있는 교차 모듈 영향 범위를 review에 명시했는가?
- [ ] 증거 없는 승인이나 verification debt를 사실상 숨기고 있지 않은가?
- [ ] read-only / write boundary, governance boundary, human-review-required scope를 무심코 넘지 않았는가?
...
- [ ] `Self-Review Summary`의 verification result와 실제 evidence가 모순되지 않는가?

### 10. Dependency / License 후속 확인
...
- AI-specific risk가 없더라도 duplication debt, evidence gap, boundary drift를 봤는지 잔여 리스크에 남긴다.
