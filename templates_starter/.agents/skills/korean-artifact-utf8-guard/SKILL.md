...
Use this skill only at the risky points named in the description. The goal is to prevent mojibake such as `테스트 메모` turning into broken text, which happens when UTF-8 Korean text is read through a non-UTF-8 path and then written back. Treat artifact and rule documents as UTF-8 without BOM by default, even when the current edit looks ASCII-only.
