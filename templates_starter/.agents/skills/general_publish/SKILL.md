...
전용 provider 스킬이 아직 없는 배포 대상은 이 스킬을 fallback으로 사용합니다.
...
- `Deployment Provider`, `Preferred Deployment Skill`, `Fallback When No Dedicated Skill`가 정리되어 있어야 합니다.

## 사용자 오더 준수
- 사용자가 명령어만 요청했다면 명령어만 제공하고 실제 배포는 하지 않습니다.
- 사용자가 `build only` 또는 `deploy only`를 지시했다면 그 범위를 넘어서지 않습니다.
- provider-specific skill이 없는 상태라고 해서 즉석에서 새 스킬을 만들지 않고, 승인된 명령과 fallback 경로만 사용합니다.
...
- 현재 provider에 전용 스킬이 있는지 먼저 확인하고, 없을 때만 이 스킬을 fallback으로 사용합니다.
- Vercel, Supabase, Oracle Cloud처럼 아직 전용 스킬이 없는 provider는 `DEPLOYMENT_PLAN.md`에 적힌 승인 명령만 사용합니다.
- GitHub release gate를 닫은 commit과 현재 배포 대상 commit이 같은지 확인합니다.
...
- `DEPLOYMENT_PLAN.md`에 GitHub release path, 사용한 deployment provider/skill, 버전, 날짜, 배포 대상, 커밋 기준점, 결과, URL, 롤백 여부를 기록합니다.
