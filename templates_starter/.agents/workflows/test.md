...
- 필요 시 `IMPLEMENTATION_PLAN.md > Current Iteration`, `Task Packet Ledger`, `Requirement Baseline`
...
- **Targeted Green:** 현재 Scope와 지정 acceptance check만 통과
- **Package Green:** 빌드/패키지/앱 단위 검증까지 통과
- **Workspace Green:** 전체 워크스페이스 회귀 관점의 검증까지 통과
- **Merge Ready:** 최신 `Requirement Baseline` 기준 검증 완료 + `Requirements Sync Check=Pass` + 필요한 green level 달성 + 수동 / 실환경 / dependency / review gate가 모두 닫힘
- **Release Pass:** 릴리즈 범위 전체 요구사항 충족 + `Merge Ready`
