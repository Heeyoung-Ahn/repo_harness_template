...
- 필요 시 `IMPLEMENTATION_PLAN.md > Current Iteration`, `Task Packet Ledger`
- UI 범위면 `UI_DESIGN.md`
...
- 광범위 실패를 재현했거나 병렬 작업이 많은 범위면 구현 전에 branch freshness를 먼저 확인합니다. `Behind / Diverged`면 바로 코드 결함으로 단정하지 않습니다.
