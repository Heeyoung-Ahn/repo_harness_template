...
이 스킬은 프로젝트 코드를 안전하게 GitHub 등 원격 저장소에 배포(Push)할 때 사용합니다.
...
### 5단계: (선택) Release 태깅
버전 릴리즈 태그는 배포 승인 또는 실제 릴리즈 시점에만 생성합니다.
```bash
...
### 완료 후
사용자에게 최신 커밋 푸시가 완료되었음을 알리고, 필요 시 다음 단계(Reviewer, DevOps, version_closeout)를 안내합니다.
