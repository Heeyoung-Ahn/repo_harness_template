...
## Quick Read
- 현재 승인된 아키텍처 스타일: document-centric governance core + optional enterprise-governed pack + separate `Project Monitor Web` + optional OMX sidecar compatibility
- 현재 반영된 Requirement Baseline / 변경 영향: `Scalable Governance Profiles v0.3` 승인본에 맞춰 `governance_controls.json`, enterprise-governed pack, optional `.omx/*` sidecar, skeptical evaluator gate를 고정했다.
- 핵심 도메인 경계: Governance Core / Profile Contract / Enterprise Governance Pack / Parser & Projection / Project Monitor Web / Integration Adapters
- 이번 범위에서 건드리는 폴더/모듈: `.agents/artifacts/*`, `.agents/rules/*`, `.agents/runtime/{team.json,governance_controls.json}`, `tools/project-monitor-web/*`, starter/reset pack source
- 상태와 데이터의 주인: artifact와 runtime contract가 truth를 유지하고, monitor와 `.omx/*`는 파생 projection 또는 보조 상태만 가진다.
- 다음 역할이 꼭 지켜야 할 구조 규칙: starter는 OMX나 sandbox runtime에 의존하지 않으며, enterprise 규율은 `enterprise_governed` overlay와 `governance_controls.json`으로만 켠다.
- 이번 문서의 리뷰 포인트: pack activation rule, protected path/HITL contract, `.omx` truth boundary, workflow compatibility mapping, critical-domain verification gate
...
- Last Requirement Sync At: 2026-04-07 10:33
...
- 주요 도메인: Governance Core, Profile Contract, Enterprise Governance Pack, Parser & Projection, Project Monitor Web, Integration Adapters
- 핵심 설계 원칙:
