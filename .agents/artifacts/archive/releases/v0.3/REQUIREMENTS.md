...
## Quick Read
- 이번 문서의 핵심 목표: `one core, multiple profiles`를 유지한 채 `Core + Enterprise Pack` 전략을 도입하고, `RHT outer loop / optional OMX orchestration loop / generation-evaluation inner loop`를 깨지 않는 표준 계약으로 고정한다.
- 이번 버전의 꼭 필요한 결과: ① `solo/team/large/governed` core profile 유지 ② `enterprise_governed` optional pack 정의 ③ `.agents/runtime/team.json`과 `.agents/runtime/governance_controls.json` 계약 고정 ④ `Project Monitor Web`의 read-only 경계 유지 ⑤ starter/reset source에 enterprise pack placeholder 추가 ⑥ optional `.omx/*` sidecar 호환 규칙 고정
- 이번 버전에서 하지 않을 것: OMX runtime을 truth로 승격, starter 기본 orchestration 의존, write action monitor, default container/read-only sandbox, 새 top-level `CLAUDE.md` truth 도입
- 사용자가 최종 승인한 범위: 현재 배포 focus는 유지하고, 하이브리드 통합은 `CR-02 Enterprise Hybrid Harness`로 반영한다.
- 현재 승인 기준선과 마지막 변경 요약: `Scalable Governance Profiles v0.3` 승인본. `v0.2`의 profile/schema/monitor 경계를 유지하면서 enterprise-governed pack, governance controls, OMX compatibility, skeptical evaluator gate를 추가했다.
- 현재 남아 있는 큰 질문: 없음. 현재 release blocker는 여전히 첫 self-hosting preview target 선택과 bring-up evidence다.
- 다음 역할이 꼭 읽어야 할 포인트: `.agents/*`와 runtime contract가 계속 truth이며, `.omx/*`는 self-hosting optional sidecar일 뿐이다.

## Status
- Document Status: Approved
- Owner: Planner
- Current Requirement Baseline: Scalable Governance Profiles v0.3
- Requirements Sync Status: In Sync
- Last Requirement Change At: 2026-04-07 10:33
...
- 현재 release blocker는 preview bring-up이며, `CR-02`는 그 blocker를 재정의하지 않는다.
