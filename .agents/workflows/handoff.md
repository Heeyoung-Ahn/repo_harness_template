...
10. If handoff is caused by `Needs User Decision` or `manual gate pending`, ask in the current session first and record the gate in artifacts. If the user stays unresponsive or may be away, then use the `remote_approval_notify` skill or `.agents/scripts/invoke_user_gate.ps1` in `local-first` mode for mobile fallback.
11. rules / workflows / artifacts를 수정했다면 `powershell -ExecutionPolicy Bypass -File ".agents/scripts/check_harness_docs.ps1"`를 실행해 구조 위반이 없는지 확인한다.
